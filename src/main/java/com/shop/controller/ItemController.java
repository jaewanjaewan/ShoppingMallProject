...
public class ItemController {
    ...
    public String itemNew(@Valid ItemFormDto itemFormDto, BindingResult bindingResult,
        ...
        //상품 등록 시 첫번째이미지(메인페이지에서 보여줄 메인이미지)가 없다면 에러 메시지와 함께 상품 등록 페이지로 전환
        ...
        try {
            //상품 저장 로직 호출. 상품 정보와 상품 이미지 정보를 담고 있는 itemImgFileList를 넘겨줌
    ...
    public String itemDtl(@PathVariable("itemId") Long itemId, Model model){
        ...
        try { //조회한 상품 데이터를 모델에 담아서 뷰로 전달
            ItemFormDto itemFormDto = itemService.getItemDtl(itemId);
            model.addAttribute("itemFormDto", itemFormDto);
        } catch(EntityNotFoundException e){ //상품이 존재하지 않을경우 에러메시지를 담아서 상품 등록 페이지로 이동
